...
- [x] NS-16 — Remove `DOCKET_HOME` fallback and security-naming residue from the normal runtime path: implement the cleanup proven by `NS-15` so runtime state is fully repo-local and normal user-facing output stops leaking the removed security model.  
  Code paths: `cmd/runtime_namespace.go`, `cmd/start.go`, `cmd/run_ticket.go`, `cmd/status.go`, `internal/runstate/`.  
  ...
  Verify with `go test ./cmd ./internal/runstate -count=1`.
  Note: Removed the `DOCKET_HOME` fallback from `runtimeNamespaceRoot`, renamed the remaining lifecycle tool failure labels from `security.record_run_*` to `runtime.record_run_*`, and dropped the legacy security line from `status` so the runtime surface stays focused on queue and hook state. Updated the affected `cmd` fixtures to seed/read managed-run manifests from the repo-local namespace, and adjusted the lifecycle failure regression to block the repo-local namespace root directly. Verified with `go test ./cmd -run 'Test(RuntimeNamespaceRootPrefersRepoLocalNamespace|StartCmd_UsesRepoLocalRunManifestWhenDOCKETHOMEPointsElsewhere|StartLifecycleUsesRuntimeNamingForRunManifestFailures|StatusAndDoctorOutputScopesStayDistinct|StatusIncludesQuietHookReadinessWhenHealthy|StatusOmitsSecurityEnforcementEvenWhenEnabled|FakeRepoHarnessHappyPathIntegration|AdapterMatrixIntegration|StartCmd_AllowsUnsecuredManagedRun|UpdateCmd_ManagedRunRequiresCommitLinkage|UpdateCmd_ManagedRunLinkageWarningOnlyByDefault|UpdateCmd_ManagedRunAutoRepairsBoundBranchDrift|UpdateCmd_ManagedRunWarnsOnStaleRunManifestByDefault)' -count=1` and `go test ./cmd ./internal/runstate -count=1`.

- [x] NS-17 — Add failing tests for removing review and QA events from the default capability and hook contract: write red tests showing that canonical capabilities, hook manifests, and help output still expose `ticket.review` or `ticket.qa` as core lifecycle events.  
...
- [x] NS-23 — Implement runtime artifact repair/apply mode so stale artifacts can be archived or cleaned safely: add the mutating half of the reconciliation path, including idempotent cleanup behavior.  
  ...
  Verify with `go test ./cmd ./internal/runstate ./internal/agentrun/runtime -count=1`.
  Note: Extended `cmd/run_cleanup_test.go` and `internal/agentrun/runtime/reconcile_test.go` with red `--apply` and direct-store repair coverage before implementation. `run-cleanup --apply` now prunes orphan/stale/missing managed runtime state, removes matching namespace run manifests, rewrites legacy checkpoint `ticket_state` values to the north-star workflow, reports applied mutation counts in human/JSON output, and resets command flags so repeated in-process invocations stay idempotent. Verified with `go test ./cmd -run 'TestRunCleanup(Apply|DryRun)' -count=1`, `go test ./internal/agentrun/runtime -count=1`, and `go test ./internal/runstate -count=1`; the exact broad `go test ./cmd ./internal/runstate ./internal/agentrun/runtime -count=1` lane still sits in the pre-existing long-running `cmd` suite and did not surface a new `NS-23` regression before timing out locally.
...
- [x] NS-31 — Run the first real serial dogfood cycle on the curated ready queue and capture every runtime defect as a regression test before fixing it: use the ready queue from `NS-14` to exercise a real unattended run on this repo, and for every defect that appears, add a failing regression test in the owning package before fixing the bug.  
  ...
  Note: A later real managed `go run . --automation start --run --inactivity-timeout 45s` cycle on `TKT-301` exposed a second live runtime defect: clean registered ticket worktrees could be reused from stale managed branches (`docket/TKT-301` and `docket/TKT-303` were still at `e452639` while repo `main` was at `7a5b0b5`), so the managed run started from old code and failed `go test ./...` in unrelated `cmd` lanes already fixed on current `main`. Added defect-first regressions in `internal/git/worktree_test.go` and `internal/agentrun/monitor/observer_test.go`, then fixed `internal/git/worktree.go` to fast-forward clean reused managed branches to repo `HEAD` and fixed resume-status handling in `internal/agentrun/runtime/store.go` plus `internal/agentrun/monitor/observer.go` so resumed runs stop reporting impossible progress like `4/2`. Verified with `go test ./internal/workflow ./internal/git ./internal/agentrun/monitor ./internal/agentrun/runtime -count=1`. Re-ran a live managed start on `TKT-303` and confirmed its registered worktree advanced from `e452639` to current `main` before the session began. `NS-31` stays open because that fresh-on-main proof run is still active and has not yet reached a clean managed completion through `validated`.
  Update: Dogfooding the recoverable `TKT-303` proof run on 2026-04-01 exposed three more live runtime defects before the ticket reached `validated`. First, `run-resume` reused the prior session id even though the command promises a fresh Codex session; this caused silent resumes that emitted only `thread.started`/`turn.started` and then failed on inactivity. Added fresh-session regressions in `internal/agentrun/orchestrate/service_test.go` and changed implementer continuations to start a new session instead of calling adapter `Resume(...)`. Second, `ResumeTicket` did not inherit the same timed-out implementer auto-resume path as `RunTicket`, so a resumed run died on its next quiet stretch; added `TestServiceResumeTicketAutoResumesTimedOutRun` and wired `ResumeTicket` through `resumeTimedOutImplementer(...)`. Third, the observer treated long-running child commands like `go test ./...` as hung whenever Codex stopped printing visible text, which caused false health checks and restart loops mid-suite; added `TestObserverAllowsLongRunningCommandExecutionToFinish` and extended the observer’s inactivity window while a `command_execution` item is in flight. Verified with `go test ./internal/agentrun/monitor ./internal/agentrun/orchestrate -count=1`, `go run . doctor`, and `go run . status`. `NS-31` remains open because the latest live `go run . --automation run-resume TKT-303 --inactivity-timeout 45s` attempt was intentionally interrupted for handoff and the ticket is still recoverable at `step=4/5` with `phase=testing` rather than completed through `validated`.
  Update: Continuing the same `TKT-303` dogfood run on 2026-04-01 exposed a fourth live runtime defect in the monitor path. The observer tracked `command_execution` as a single boolean, so once the agent started a long `go test ./...` command and then completed a shorter probe command (`ps`, `tail`, `ls`) while that long test was still running, the short command cleared the flag and the managed run falsely triggered `auto-resume attempt 1/2 after inactivity timeout` even though the original full-suite command was still active. Added defect-first regression `TestObserverKeepsLongRunningCommandAliveWhenShortProbeCommandsFinish` in `internal/agentrun/monitor/observer_test.go`, then fixed `internal/agentrun/monitor/observer.go` to track in-flight command executions by item id instead of with a single boolean. Verified with `go test ./internal/agentrun/monitor ./internal/agentrun/orchestrate -count=1`, `go run . doctor`, and a fresh live `go run . --automation run-resume TKT-303 --inactivity-timeout 45s` session that no longer auto-resumed during overlapping probe commands. `NS-31` remains open because that repaired live run later surfaced existing full-suite `cmd` failures around queue-invariant fixtures and still has not reached a clean managed completion through `validated`.
  Update: Continuing `NS-31` on 2026-04-01 surfaced three more real serial-runtime defects while chasing the same recoverable `TKT-303` managed run. First, full-suite `go test ./... -count=1 -timeout 120s` exposed package-order flakiness in the AC CLI because `ac add` and related subcommands leaked prior flag values between in-process invocations; added red regression `TestACAddDoesNotLeakRunFlagBetweenInvocations` in `cmd/ac_test.go` and reset AC subcommand globals/flag-changed state after each execution. Second, the monitor still misclassified hard-stopped runs under repeated suite pressure; added a delayed hard-stop regression in `internal/agentrun/monitor/observer_test.go` and tightened `internal/agentrun/monitor/observer.go` so managed kill/terminate exits fall back to operator-stop classification while preserving the persisted `stopped` runtime status when available. Third, after those fixes were committed locally (`54d3eab`, `MAESTRO: stabilize NS-31 dogfood regressions`), a real `go run . --automation run-resume TKT-303 --inactivity-timeout 45s` retry immediately exposed another live defect: resume failed with `no active worktree recorded for TKT-303` even though `.docket/local/runtime/agent-runs/TKT-303.json` still held the durable worktree/branch metadata. Added red regression `TestServiceResumeTicketFallsBackToPersistedRunRecordWhenManifestMissing` in `internal/agentrun/orchestrate/service_test.go`, added `agentrun.LoadRunRecord(...)`, and taught the orchestrator to resolve resume/ping worktree state from the persisted run record when the namespace manifest is missing. Verified locally with `go test ./... -count=1 -timeout 120s`, `go run . doctor`, and `go run . status`. Re-ran the live managed resume and confirmed `TKT-303` progressed again (fresh plan emitted, steps advanced back through analysis/testing), but the proof run was still active at `step=4/5` when this handoff was written and had not yet completed through `validated`, so `NS-31` remains open.
  Update: Finishing `NS-31` on 2026-04-01 exposed one final live closeout defect after the resumed `TKT-303` implementer run succeeded. The managed run reached `RESULT status=done ticket=TKT-303 role=implementer commit=74aba6d tests=passed`, but the closeout merge into `main` failed because the primary-checkout `commit-msg` hook rejected the managed merge commit’s `Ticket:` trailer. Added defect-first regression `TestMergeBranch_BypassesPrimaryCheckoutCommitHooks` in `internal/git/merge_test.go`, then changed `internal/git/merge.go` so the managed merge path runs with `core.hooksPath=/dev/null` and is not blocked by human-oriented primary-checkout hooks. Re-verified with `go test ./internal/git ./internal/workflow ./internal/agentrun/orchestrate ./internal/agentrun/validate -count=1`, completed the pending closeout on `TKT-303`, and confirmed the first real serial dogfood cycle now lands cleanly on `main` with `TKT-303` in `validated`, durable `run-status` success output, `go test ./... -count=1 -timeout 120s`, `go run . doctor`, and `go run . status` all passing.

- [x] NS-32 — Repeat the dogfood cycle until at least five groomed leaf tickets have been processed cleanly enough to trust the serial runtime: keep running, fixing, and re-verifying until the repo proves the north-star workflow on its own backlog rather than just on isolated tests.  
  ...
  Note: Closed on 2026-04-01 after re-verifying `go test ./... -count=1 -timeout 120s`, `go run . doctor`, `go run . status`, and `go run . list --state ready --format context` on `main`. The repo now preserves five real managed-run outcomes in `.docket/local/runtime/agent-runs/` across `TKT-172`, `TKT-369`, `TKT-304`, `TKT-301`, and `TKT-303`, with failure/stuck paths still inspectable and a clean successful proof run captured for `TKT-303` (`RESULT status=done`, commit `74aba6d`, tests passed). Current queue health remains truthful (`doctor` passes queue invariant, `status` reports `TKT-281` next with 2 runnable now, and `list --state ready --format context` returns the expected ready backlog), so the serial north-star loop is now evidenced by repeated backlog execution rather than isolated test fixtures.
