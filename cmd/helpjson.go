		...
		manifest := map[string]any{
			...
			"agent_instructions": map[string]any{
				...
				"workflow": map[string]any{
					...
					"finish":                fmt.Sprintf("docket update TKT-NNN --state %s && docket session compress TKT-NNN", completedWorkflowState(cfg)),
...
func commandOutputShape(name string) map[string]any {
	shapes := map[string]map[string]any{
		...
		"smart-commit":     {"human": "closeout-ready commit guidance with ticket trailer", "json": map[string]string{"ticket_id": "string", "ready": "bool", "commit_message": "string", "git_command": "string"}},
