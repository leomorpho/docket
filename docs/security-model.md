...
- **States and transitions:** Configuration in `.docket/config.json` defines workflow states, their semantic roles (`active`, `review`, `completed`, etc.), and `next` links. Default examples often look like `todo → in-progress → in-review → done`, but repositories may rename or expand these states. `docket update` enforces the configured graph during CLI use.
- **Unsecured execution mode:** `docket start` may begin an agent-managed run even when no approved `workflow.lock` is active. In that case the run manifest is recorded without a workflow hash, worktree/claim enforcement still applies, and privileged terminal transitions remain blocked until secure mode is unlocked.
